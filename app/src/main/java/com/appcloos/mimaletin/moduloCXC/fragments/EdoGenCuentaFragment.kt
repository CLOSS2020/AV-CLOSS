...
class EdoGenCuentaFragment : Fragment() {
    ...
    lateinit var nombreEmpresa:String; lateinit var enlaceEmpresa:String; lateinit var codigoSucursal:String
    ...
    lateinit var fechamodifi:String; lateinit var aceptadev:String; lateinit var estatusdoc:String;
    ...
    private fun consultarEstadoClientes(Text: String?){
        ...
        //-- este query debe modificarse para poder mostrar el estatus general de cada cliente

        //println("SELECT DISTINCT  codcliente, nombrecli, SUM(dtotalfinal - dtotpagos) as montototal as montototal, MIN(vence), ROUND(JULIANDAY('now') - JULIANDAY(MIN(vence))) as fechaentregaantigua FROM ke_doccti WHERE estatusdoc < '2' AND vendedor = '${cod_usuario}' GROUP BY codcliente ORDER BY montototal desc")

        val cursor: Cursor = if (Text == null || Text.isEmpty()){
            ke_android.rawQuery(
                "SELECT DISTINCT  codcliente, nombrecli, SUM(dtotalfinal - dtotpagos) as montototal, MIN(vence), ROUND(JULIANDAY('now') - JULIANDAY(MIN(vence))) as fechaentregaantigua FROM ke_doccti WHERE estatusdoc < '2' AND vendedor = '${cod_usuario}' GROUP BY codcliente ORDER BY montototal desc",
                null
            )
        }else{
            ke_android.rawQuery(
                "SELECT DISTINCT  codcliente, nombrecli, SUM(dtotalfinal - dtotpagos) as montototal, MIN(vence), ROUND(JULIANDAY('now') - JULIANDAY(MIN(vence))) as fechaentregaantigua FROM ke_doccti WHERE estatusdoc < '2' AND vendedor = '${cod_usuario}' AND (nombrecli LIKE '%$Text%' OR codcliente LIKE '%$Text%') GROUP BY codcliente ORDER BY montototal desc",
                null
            )
        ...
        while (cursor.moveToNext()) {
            ...
            var montoTotal    = cursor.getDouble(2)
            montoTotal        = (montoTotal * 100.00).roundToInt() /100.00
            val fechaVence    = cursor.getString(4)
            val limite        = 0.00
            val saldo         = 0.00

            listaEstadoGen.add(EdoGeneralCxc(codcliente, nombreCliente, montoTotal, fechaVence, limite, saldo))
        }
        cursor.close()
        ke_android.close()
        adapter = EdoGenCuentaAdapter(edogencxc = listaEstadoGen, onClickListener = { cliente, nomCliente -> irACXC(cliente, nomCliente) })
    ...
    private fun buscarItem(Text: String?) {
        if (Text != null) {
            if (Text.isNotEmpty()) {
                val estadoFiltrado =
                    listaEstadoGen.filter { listaEstadoGen ->
